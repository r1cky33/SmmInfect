...
UINT64 ZGetWindowsProcAddressX64(UINT64 cr3, UINT64 base,
    ...
    v8 = (UINT32 *)(base + (UINT32)ReadVirtual32(
                               (UINT64)v6 + sizeof(UINT32) * 8, cr3));
    ...
    ReadVirtual(base + ReadVirtual32((UINT64)v8, cr3), cr3, (UINT8 *)name,
                (UINT64)sizeof(name));
    name[49] = 0;
    ...
    while (StrCmpi((char *)name, export_name)) {
        ...
        ReadVirtual(base + (UINT64)ReadVirtual32((UINT64)v8, cr3), cr3,
                    (UINT8 *)name, (UINT64)sizeof(name));
        name[49] = 0;
    }
    return base +
           ReadVirtual32(v7 + 4 * ReadVirtual16(v10 + 2 * (INT32)v5, cr3), cr3);
}

UINT64 GetWindowsBaseAddressModuleX64(UINT64 eprocess,
                                      unsigned short *process_name) {
    UINT64 *v3;
    UINT64 *i;
    ...
    for (i = (UINT64 *)ReadVirtual64((UINT64)v3, cr3);;
         i = (UINT64 *)ReadVirtual64((UINT64)i, cr3)) {
        if (i == v3) {
            return 0;
        }

        unsigned short name[30];
        ReadVirtual(ReadVirtual64((UINT64)i + (UINT64)sizeof(UINT64) * 10, cr3),
                    cr3, (UINT8 *)name, sizeof(name));
...
UINT64 GetWindowsSectionBaseAddressX64(UINT64 eprocess, UINT64 module_base,
...
EFI_STATUS SetupWindows(EFI_SMM_CPU_PROTOCOL *cpu,
    ...
    if (cpu == NULL || smst == NULL) {
        LOG_ERROR("[ERROR] cpu or smst were invalid\ŗ\n");
    ...
    LOG_INFO("kernel-base: 0x%x\r\n", KernelBase);

    PsInitialSystemProcess = ZGetWindowsProcAddressX64(
        KernelCr3, KernelBase, "PsInitialSystemProcess");
    if (PsInitialSystemProcess == 0) {
        return EFI_NOT_FOUND;
    }

    PsGetProcessSectionBaseAddress = ReadVirtual32(
        ZGetWindowsProcAddressX64(KernelCr3, KernelBase,
                                  "PsGetProcessSectionBaseAddress") +
            3,
        KernelCr3);
    PsGetProcessExitProcessCalled = ReadVirtual32(
        ZGetWindowsProcAddressX64(KernelCr3, KernelBase,
                                  "PsGetProcessExitProcessCalled") +
            2,
        KernelCr3);
    PsGetProcessImageFileName =
        ReadVirtual32(ZGetWindowsProcAddressX64(KernelCr3, KernelBase,
                                                "PsGetProcessImageFileName") +
                          3,
                      KernelCr3);
    ActiveProcessLinks =
        ReadVirtual32(
            ZGetWindowsProcAddressX64(KernelCr3, KernelBase, "PsGetProcessId") +
                3,
            KernelCr3) +
        8;
    PsGetProcessPeb = ReadVirtual32(
        ZGetWindowsProcAddressX64(KernelCr3, KernelBase, "PsGetProcessPeb") + 3,
        KernelCr3);
    ...
    LOG_INFO("Successfully setup Windows\r\n");
...
UINT64 GetWindowsEProcess(const char *process_name) {
    ...
    entry = proc;
    do {
        ReadVirtual(entry + PsGetProcessImageFileName, KernelCr3, (UINT8 *)name,
                    15);
        name[14] = 0;

        UINT32 exitcalled =
            ReadVirtual32(entry + PsGetProcessExitProcessCalled, KernelCr3);
...
EFI_STATUS MemGetKernelCr3(UINT64 *cr3) {
    ...
    UINT64 rip;
    UINT64 tempcr3;
    Cpu->ReadSaveState(Cpu, sizeof(tempcr3), EFI_SMM_SAVE_STATE_REGISTER_CR3,
                       GSmst2->CurrentlyExecutingCpu, (VOID *)&tempcr3);
    Cpu->ReadSaveState(Cpu, sizeof(rip), EFI_SMM_SAVE_STATE_REGISTER_RIP,
                       GSmst2->CurrentlyExecutingCpu, (VOID *)&rip);

    LOG_INFO("cr3 savedstate: 0x%x\r\n", tempcr3);
    LOG_INFO("rip savedstate: 0x%x\r\n", rip);
    LOG_INFO("cr3: 0x%x\r\n", read_cr3());
...
EFI_STATUS MemGetKernelBase(UINT64 *base) {
    ...
    UINT64 rip;
    UINT64 cr3;
    Cpu->ReadSaveState(Cpu, sizeof(cr3), EFI_SMM_SAVE_STATE_REGISTER_CR3,
                       GSmst2->CurrentlyExecutingCpu, (VOID *)&cr3);
    Cpu->ReadSaveState(Cpu, sizeof(rip), EFI_SMM_SAVE_STATE_REGISTER_RIP,
                       GSmst2->CurrentlyExecutingCpu, (VOID *)&rip);
